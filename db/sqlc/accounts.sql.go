...
const listValidCertificateAuthorityFromSubordinateCA = `-- name: ListValidCertificateAuthorityFromSubordinateCA :many
...
func (q *Queries) ListValidCertificateAuthorityFromSubordinateCA(ctx context.Context, arg ListValidCertificateAuthorityFromSubordinateCAParams) ([]interface{}, error) {
	rows, err := q.db.QueryContext(ctx, listValidCertificateAuthorityFromSubordinateCA, arg.SubordinateCa, arg.Environment)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []interface{}{}
	for rows.Next() {
		var certificate_authorities interface{}
		if err := rows.Scan(&certificate_authorities); err != nil {
			return nil, err
		}
		items = append(items, certificate_authorities)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
